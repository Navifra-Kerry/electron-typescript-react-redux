...
export class HelpMenu extends React.Component<IHelpMenuProps, IHelpMenuState> {
    ...
    public render() {
        return (
            <div className={'help-menu-button'} onClick={() => this.setState({ show: true })}>
                ...
                />
                <MessageBox title={'Electron Example Help'} message={this.getHelpBody()} show={this.state.show} onCancel={this.onClose} hideFooter={true} />
            </div>
        );
    }
    ...
    private getHelpBody = () => {
        ...
        return <div className="help-body container">{groupKeys.map((group) => (group.length ? this.getRegistrationRow(group, registrations) : null))}</div>;
    };
        ...
        for (const key of caseConsolidatedKeys) {
            ...
            if (!alreadyGrouped.has(key)) {
                ...
                for (const otherKey of caseConsolidatedKeys) {
                    if (!alreadyGrouped.has(otherKey) && this.bindingEquals(registrations[key], registrations[otherKey])) {
                        group.push(otherKey);
        ...
        if (keyRegistration) {
            return (
                <div key={keyRegistration.displayName} className={'help-key row'}>
                    <div className={`col-1 keybinding-icon ${keyRegistration.icon ? `fas ${keyRegistration.icon}` : ''}`} />
